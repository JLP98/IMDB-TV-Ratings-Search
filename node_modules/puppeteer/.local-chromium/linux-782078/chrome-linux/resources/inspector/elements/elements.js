import{ObjectWrapper,EventTarget,UIString,Revealer,Throttler,Color,Settings,Linkifier as Linkifier$2,ParsedURL,ls as ls$2,Console}from'../common/common.js';import{DOMModel,ResourceTreeModel,CSSModel,OverlayModel,CSSMetadata,CSSMatchedStyles,CSSRule,CSSStyleDeclaration,CSSMedia,ConsoleModel,RuntimeModel,SDKModel,RemoteObject}from'../sdk/sdk.js';import{PopoverHelper,ThrottledWidget,Utils,ARIAUtils,Widget,Geometry,TreeOutline,Fragment,Icon,UIUtils,KeyboardShortcut,ContextMenu,TextPrompt,Toolbar,InplaceEditor,TextEditor,Tooltip,XLink,SyntaxHighlighter,Panel,SplitWidget,SearchableView,ViewManager,View,TabbedPane}from'../ui/ui.js';import{ImagePreview,Linkifier as Linkifier$1,JSPresentationUtils}from'../components/components.js';import{ExtensionServer}from'../extensions/extensions.js';import{userMetrics,UserMetrics,Platform,InspectorFrontendHost}from'../host/host.js';import{BezierEditor,CSSShadowEditor,ColorSwatch,CSSShadowModel,SwatchPopoverHelper}from'../inline_editor/inline_editor.js';import{CSSWorkspaceBinding,ResourceUtils}from'../bindings/bindings.js';import{TextUtils,TextRange}from'../text_utils/text_utils.js';import{Spectrum,ContrastInfo}from'../color_picker/color_picker.js';import{StringUtilities}from'../platform/platform.js';import'../protocol_client/protocol_client.js';import{Runtime}from'../root/root.js';import{EventListenersView}from'../event_listeners/event_listeners.js';class ComputedStyleModel extends ObjectWrapper.ObjectWrapper{constructor(){super();this._node=self.UI.context.flavor(DOMModel.DOMNode);this._cssModel=null;this._eventListeners=[];self.UI.context.addFlavorChangeListener(DOMModel.DOMNode,this._onNodeChanged,this);}
...
var ComputedStyleModel$1=Object.freeze({__proto__:null,ComputedStyleModel:ComputedStyleModel,Events:Events,ComputedStyle:ComputedStyle});class ImagePreviewPopover{constructor(container,getLinkElement,getDOMNode){this._getLinkElement=getLinkElement;this._getDOMNode=getDOMNode;this._popover=new PopoverHelper.PopoverHelper(container,this._handleRequest.bind(this));this._popover.setHasPadding(true);this._popover.setTimeout(0,100);}
_handleRequest(event){const link=this._getLinkElement(event);if(!link||!link[HrefSymbol]){return null;}
return{box:link.boxInWindow(),hide:undefined,show:async popover=>{const node=this._getDOMNode((link));if(!node){return false;}
const precomputedFeatures=await ImagePreview.ImagePreview.loadDimensionsForNode(node);const preview=await ImagePreview.ImagePreview.build(node.domModel().target(),link[HrefSymbol],true,{precomputedFeatures});if(preview){popover.contentElement.appendChild(preview);}
return!!preview;}};}
hide(){this._popover.hidePopover();}
static setImageUrl(element,url){element[HrefSymbol]=url;return element;}}
const HrefSymbol=Symbol('ImagePreviewPopover.Href');class PlatformFontsWidget extends ThrottledWidget.ThrottledWidget{constructor(sharedModel){super(true);this.registerRequiredCSS('elements/platformFontsWidget.css');this._sharedModel=sharedModel;this._sharedModel.addEventListener(Events.ComputedStyleChanged,this.update,this);this._sectionTitle=document.createElement('div');this._sectionTitle.classList.add('title');this.contentElement.classList.add('platform-fonts');this.contentElement.appendChild(this._sectionTitle);this._sectionTitle.textContent=UIString.UIString('Rendered Fonts');this._fontStatsSection=this.contentElement.createChild('div','stats-section');}
...
var PlatformFontsWidget$1=Object.freeze({__proto__:null,PlatformFontsWidget:PlatformFontsWidget});const decorateNodeLabel=function(node,parentElement,tooltipContent){const originalNode=node;const isPseudo=node.nodeType()===Node.ELEMENT_NODE&&node.pseudoType();if(isPseudo&&node.parentNode){node=node.parentNode;}
...
const root=document.createElement('span');root.classList.add('monospace');const shadowRoot=Utils.createShadowRootWithCoreStyles(root,'elements/domLinkifier.css');const link=shadowRoot.createChild('div','node-link');decorateNodeLabel(node,link,options.tooltip);link.addEventListener('click',()=>Revealer.reveal(node,false)&&false,false);link.addEventListener('mouseover',node.highlight.bind(node,undefined),false);link.addEventListener('mouseleave',()=>OverlayModel.OverlayModel.hideDOMNodeHighlight(),false);if(!options.preventKeyboardFocus){link.addEventListener('keydown',event=>isEnterKey(event)&&Revealer.reveal(node,false)&&false);link.tabIndex=0;ARIAUtils.markAsLink(link);}
...
function restoreURLs(fieldValue,modelValue){const splitFieldValue=fieldValue.split(CSSMetadata.URLRegex);if(splitFieldValue.length===1){return fieldValue;}
const modelUrlRegex=new RegExp(CSSMetadata.URLRegex);for(let i=1;i<splitFieldValue.length;i+=2){const match=modelUrlRegex.exec(modelValue);if(match){splitFieldValue[i]=match[0];}}
...
return false;}
static createPropertyFilterElement(placeholder,container,filterCallback){const input=document.createElement('input');input.placeholder=placeholder;function searchHandler(){const regex=input.value?new RegExp(input.value.escapeForRegExp(),'i'):null;filterCallback(regex);}
...
index++;}}
return-1;}
continueEditingElement(sectionIndex,propertyIndex){const section=this.allSections()[sectionIndex];if(section){const element=section.closestPropertyForEditing(propertyIndex);if(!element){section.element.focus();UIUtils.markAsFocusedByKeyboard(section.element);return;}
...
const focusedIndex=this.focusedSectionIndex();this._linkifier.reset();this._sectionBlocks=[];const node=this.node();if(!matchedStyles||!node){this._noMatchesElement.classList.remove('hidden');return;}
this._sectionBlocks=await this._rebuildSectionsForMatchedStyleRules((matchedStyles));this._sectionsContainer.removeChildren();const fragment=document.createDocumentFragment();let index=0;let elementToFocus=null;for(const block of this._sectionBlocks){const titleElement=block.titleElement();if(titleElement){fragment.appendChild(titleElement);}
for(const section of block.sections){fragment.appendChild(section.element);if(index===focusedIndex){elementToFocus=section.element;}
index++;}}
this._sectionsContainer.appendChild(fragment);if(elementToFocus){elementToFocus.focus();}
...
async _rebuildSectionsForMatchedStyleRules(matchedStyles){if(this._idleCallbackManager){this._idleCallbackManager.discard();}
...
const lastBlock=blocks.peekLast();this._idleCallbackManager.schedule(()=>{const section=new StylePropertiesSection(this,matchedStyles,style);lastBlock.sections.push(section);});}
let pseudoTypes=[];const keys=matchedStyles.pseudoTypes();if(keys.delete(Protocol.DOM.PseudoType.Before)){pseudoTypes.push(Protocol.DOM.PseudoType.Before);}
pseudoTypes=pseudoTypes.concat([...keys].sort());for(const pseudoType of pseudoTypes){const block=SectionBlock.createPseudoTypeBlock(pseudoType);for(const style of matchedStyles.pseudoStyles(pseudoType)){this._idleCallbackManager.schedule(()=>{const section=new StylePropertiesSection(this,matchedStyles,style);block.sections.push(section);});}
blocks.push(block);}
for(const keyframesRule of matchedStyles.keyframes()){const block=SectionBlock.createKeyframesBlock(keyframesRule.name().text);for(const keyframe of keyframesRule.keyframes()){this._idleCallbackManager.schedule(()=>{block.sections.push(new KeyframePropertiesSection(this,matchedStyles,keyframe.style));});}
blocks.push(block);}
await this._idleCallbackManager.awaitDone();return blocks;}
...
willHide(){this._swatchPopoverHelper.hide();this._imagePreviewPopover.hide();super.willHide();}
...
const ruleLocation=this._getRuleLocationFromCSSRule(rule);const header=rule.styleSheetId?matchedStyles.cssModel().styleSheetHeaderForId(rule.styleSheetId):null;if(header&&header.isMutable){const label=header.isInline?'<style>':UIString.UIString('constructed stylesheet');if(header.ownerNode){const link=linkifyDeferredNodeReference(header.ownerNode);link.textContent=label;return link;}
return createTextNode(label);}
if(ruleLocation&&rule.styleSheetId&&header&&!header.isAnonymousInlineStyleSheet()){return StylePropertiesSection._linkifyRuleLocation(matchedStyles.cssModel(),linkifier,rule.styleSheetId,ruleLocation);}
...
return fragment;}
_createSelectorElement(text,isMatching,navigationIndex){const element=document.createElement('span');element.classList.add('simple-selector');element.classList.toggle('selector-matches',isMatching);if(typeof navigationIndex==='number'){element._selectorIndex=navigationIndex;}
...
closestPropertyForEditing(propertyIndex){const rootElement=this.propertiesTreeOutline.rootElement();if(propertyIndex>=rootElement.childCount()){return rootElement.lastChild();}
...
const link=ImagePreviewPopover.setImageUrl(Linkifier$1.Linkifier.linkifyURL(hrefUrl||url,{text:url,preventClick:true,bypassURLTrimming:true,}),hrefUrl||url);container.appendChild(link);container.createTextChild(')');return container;}}
...
_processComputedColor(text){const color=Color.Color.parse(text);if(!color){return createTextNode(text);}
const swatch=ColorSwatch.ColorSwatch.create();color.setFormat(Color.Format.RGB);swatch.setColor(color);swatch.setFormat(Color.Format.RGB);return swatch;}
...
const hadFocus=this._propertiesOutline.element.hasFocus();this._imagePreviewPopover.hide();this._propertiesOutline.removeChildren();this._linkifier.reset();const cssModel=this._computedStyleModel.cssModel();if(!nodeStyle||!matchedStyles||!cssModel){this._noMatchesElement.classList.remove('hidden');return;}
...
_measureElementSizes(){const crumbs=this.crumbsElement;const collapsedElement=document.createElement('span');collapsedElement.classList.add('crumb');collapsedElement.classList.add('collapsed');crumbs.insertBefore(collapsedElement,crumbs.firstChild);const available=crumbs.offsetWidth;const collapsed=collapsedElement.offsetWidth;const normalSizes=[];for(let i=1;i<crumbs.childNodes.length;++i){const crumb=crumbs.childNodes[i];normalSizes[i-1]=crumb.offsetWidth;}
...
const Events$1={NodeSelected:Symbol('NodeSelected')};var ElementsBreadcrumbs$1=Object.freeze({__proto__:null,ElementsBreadcrumbs:ElementsBreadcrumbs,Events:Events$1});const ls$1=ls$2;const AdornerCategories={Security:'Security',Layout:'Layout',Default:'Default',};Object.freeze(AdornerCategories);const template=document.createElement('template');template.innerHTML=`
  <style>
    :host {
      display: inline-flex;
    }
...
`;class Adorner extends HTMLElement{static create(content,name,options={}){const{category=AdornerCategories.Default}=options;const adorner=(document.createElement('devtools-adorner'));content.slot='content';adorner.append(content);adorner.name=name;adorner.category=category;return adorner;}
constructor(){super();const shadowRoot=this.attachShadow({mode:'open'});shadowRoot.appendChild(template.content.cloneNode(true));this.name='';this.category=AdornerCategories.Default;this._isToggle=false;this._ariaLabelDefault=ls$1`adorner`;this._ariaLabelActive=ls$1`adorner active`;}
connectedCallback(){if(!this.getAttribute('aria-label')){ARIAUtils.setAccessibleName(this,ls$1`${this.name} adorner`);}}
toggle(){if(!this._isToggle){return;}
const shouldBePressed=this.getAttribute('aria-pressed')==='false';ARIAUtils.setPressed(this,shouldBePressed);ARIAUtils.setAccessibleName(this,shouldBePressed?this._ariaLabelActive:this._ariaLabelDefault);}
show(){this.classList.remove('hidden');}
hide(){this.classList.add('hidden');}
addInteraction(action,options={}){const{isToggle=false,shouldPropagateOnKeydown=false,ariaLabelDefault,ariaLabelActive}=options;this.addEventListener('click',action);this._isToggle=isToggle;if(ariaLabelDefault){this._ariaLabelDefault=ariaLabelDefault;ARIAUtils.setAccessibleName(this,ariaLabelDefault);}
if(isToggle){ARIAUtils.setPressed(this,false);this.addEventListener('click',this.toggle);if(ariaLabelActive){this._ariaLabelActive=ariaLabelActive;}}
this.classList.add('clickable');ARIAUtils.markAsButton(this);this.tabIndex=0;this.addEventListener('keydown',event=>{if(event.code==='Enter'||event.code==='Space'){this.click();if(!shouldPropagateOnKeydown){event.stopPropagation();}}});}}
self.customElements.define('devtools-adorner',Adorner);var Adorner$1=Object.freeze({__proto__:null,AdornerCategories:AdornerCategories,Adorner:Adorner});const fullQualifiedSelector=function(node,justSelector){if(node.nodeType()!==Node.ELEMENT_NODE){return node.localName()||node.nodeName().toLowerCase();}
...
this._visible=visible;if(!this._visible){this._imagePreviewPopover.hide();if(this._multilineEditing){this._multilineEditing.cancel();}
return;}
...
_reset(){this.rootDOMNode=null;this.selectDOMNode(null,false);this._imagePreviewPopover.hide();delete this._clipboardNodeData;OverlayModel.OverlayModel.hideDOMNodeHighlight();this._updateRecords.clear();}
...
insertChildElement(treeElement,child,index,isClosingTag){const newElement=this._createElementTreeElement(child,isClosingTag);treeElement.insertChild(newElement,index);return newElement;}
...
var MarkerDecorator$1=Object.freeze({__proto__:null,MarkerDecorator:MarkerDecorator,GenericDecorator:GenericDecorator});class ElementsTreeElement extends TreeOutline.TreeElement{constructor(node,isClosingTag){super();this._node=node;this._gutterContainer=this.listItemElement.createChild('div','gutter-container');this._gutterContainer.addEventListener('click',this._showContextMenu.bind(this));const gutterMenuIcon=Icon.Icon.create('largeicon-menu','gutter-menu-icon');this._gutterContainer.appendChild(gutterMenuIcon);this._decorationsElement=this._gutterContainer.createChild('div','hidden');this._isClosingTag=isClosingTag;if(this._node.nodeType()===Node.ELEMENT_NODE&&!isClosingTag){this._canAddAttributes=true;}
this._searchQuery=null;this._expandedChildrenLimit=InitialChildrenLimit;this._decorationsThrottler=new Throttler.Throttler(100);if(!isClosingTag){this._adornerContainer=this.listItemElement.createChild('div','adorner-container hidden');this._adorners=[];this._adornersThrottler=new Throttler.Throttler(100);if(Runtime.experiments.isEnabled('cssGridFeatures')){this._updateStyleAdorners();}}
...
isClosingTag(){return!!this._isClosingTag;}
node(){return this._node;}
...
onbind(){if(!this._isClosingTag){this._node[this.treeOutline.treeElementSymbol()]=this;}}
onunbind(){if(this._editing){this._editing.cancel();}
if(this._node[this.treeOutline.treeElementSymbol()]===this){this._node[this.treeOutline.treeElementSymbol()]=null;}}
...
onexpand(){if(this._isClosingTag){return;}
this.updateTitle();}
oncollapse(){if(this._isClosingTag){return;}
this.updateTitle();}
...
ondblclick(event){if(this._editing||this._isClosingTag){return false;}
...
const tagName=tagNameElement.textContent;if(EditTagBlocklist.has(tagName.toLowerCase())){return false;}
...
const highlightElement=createElement('span');highlightElement.className='highlight';highlightElement.appendChild(nodeInfo);this.title=highlightElement;this.updateDecorations();this.listItemElement.insertBefore(this._gutterContainer,this.listItemElement.firstChild);if(!this._isClosingTag){this.listItemElement.appendChild(this._adornerContainer);}
delete this._highlightResult;delete this.selectionElement;delete this._hintElement;if(this.selected){this._createSelection();this._createHint();}}
...
const link=node.nodeName().toLowerCase()==='a'?XLink.XLink.create(rewrittenHref,value,'',true):Linkifier$1.Linkifier.linkifyURL(rewrittenHref,{text:value,preventClick:true});return ImagePreviewPopover.setImageUrl(link,rewrittenHref);}
...
const tagName=node.nodeNameInCorrectCase();if(this._isClosingTag){this._buildTagDOM(titleDOM,tagName,true,true,updateRecord);break;}
...
adornText(text,category){const adornerContent=(document.createElement('span'));adornerContent.textContent=text;const options={category,};const adorner=Adorner.create(adornerContent,text,options);this._adorners.push(adorner);this._updateAdorners();return adorner;}
removeAdorner(adornerToRemove){const adorners=this._adorners;adornerToRemove.remove();for(let i=0;i<adorners.length;++i){if(adorners[i]===adornerToRemove){adorners.splice(i,1);this._updateAdorners();return;}}}
removeAllAdorners(){for(const adorner of this._adorners){adorner.remove();}
this._adorners=[];this._updateAdorners();}
_updateAdorners(){this._adornersThrottler.schedule(this._updateAdornersInternal.bind(this));}
_updateAdornersInternal(){const adorners=this._adorners;const adornerContainer=this._adornerContainer;if(adorners.length===0){adornerContainer.classList.add('hidden');return Promise.resolve();}
adorners.sort(adornerComparator);adornerContainer.removeChildren();for(const adorner of adorners){adornerContainer.appendChild(adorner);}
adornerContainer.classList.remove('hidden');return Promise.resolve();}
async _updateStyleAdorners(){const node=this.node();const styles=await node.domModel().cssModel().computedStylePromise(node.id);if(!styles){return;}
if(styles.get('display')==='grid'){const gridAdorner=this.adornText('grid',AdornerCategories.Layout);gridAdorner.classList.add('grid');}}}
const InitialChildrenLimit=500;const ForbiddenClosingTagElements=new Set(['area','base','basefont','br','canvas','col','command','embed','frame','hr','img','input','keygen','link','menuitem','meta','param','source','track','wbr']);const EditTagBlocklist=new Set(['html','head','body']);const OrderedAdornerCategories=[AdornerCategories.Security,AdornerCategories.Layout,AdornerCategories.Default,];const AdornerCategoryOrder=new Map(OrderedAdornerCategories.map((category,idx)=>[category,idx+1]));function adornerComparator(adornerA,adornerB){const orderA=AdornerCategoryOrder.get(adornerA.category)||Number.POSITIVE_INFINITY;const orderB=AdornerCategoryOrder.get(adornerB.category)||Number.POSITIVE_INFINITY;if(orderA===orderB){return adornerA.name.localeCompare(adornerB.name);}
return orderA-orderB;}
var ElementsTreeElement$1=Object.freeze({__proto__:null,ElementsTreeElement:ElementsTreeElement,InitialChildrenLimit:InitialChildrenLimit,ForbiddenClosingTagElements:ForbiddenClosingTagElements,EditTagBlocklist:EditTagBlocklist});class ElementsTreeElementHighlighter{constructor(treeOutline){this._throttler=new Throttler.Throttler(100);this._treeOutline=treeOutline;this._treeOutline.addEventListener(TreeOutline.Events.ElementExpanded,this._clearState,this);this._treeOutline.addEventListener(TreeOutline.Events.ElementCollapsed,this._clearState,this);this._treeOutline.addEventListener(ElementsTreeOutline.Events.SelectedNodeChanged,this._clearState,this);SDKModel.TargetManager.instance().addModelListener(OverlayModel.OverlayModel,OverlayModel.Events.HighlightNodeRequested,this._highlightNode,this);SDKModel.TargetManager.instance().addModelListener(OverlayModel.OverlayModel,OverlayModel.Events.InspectModeWillBeToggled,this._clearState,this);}
...
if(typeof searchResult.node==='undefined'){searchResult.domModel.searchResult(searchResult.index).then(node=>{searchResult.node=node;const highlightRequestValid=this._searchConfig&&this._currentSearchResultIndex&&this._searchResults;if(highlightRequestValid){this._highlightCurrentSearchResult();}});return;}
...
_initializeSidebarPanes(splitMode){this._splitWidget.setVertical(splitMode===_splitMode.Vertical);this.showToolbarPane(null,null);const matchedStylePanesWrapper=new Widget.VBox();matchedStylePanesWrapper.element.classList.add('style-panes-wrapper');this._stylesWidget.show(matchedStylePanesWrapper.element);this._setupTextSelectionHack(matchedStylePanesWrapper.element);const computedStylePanesWrapper=new Widget.VBox();computedStylePanesWrapper.element.classList.add('style-panes-wrapper');this._computedStyleWidget.show(computedStylePanesWrapper.element);const tabSelected=event=>{const tabId=(event.data.tabId);if(tabId===UIString.UIString('Computed')){this._metricsWidget.show(computedStylePanesWrapper.element,this._computedStyleWidget.element);}else if(tabId===UIString.UIString('Styles')){this._metricsWidget.show(matchedStylePanesWrapper.element);}};this.sidebarPaneView=ViewManager.ViewManager.instance().createTabbedLocation(()=>ViewManager.ViewManager.instance().showView('elements'));const tabbedPane=this.sidebarPaneView.tabbedPane();if(this._splitMode!==_splitMode.Vertical){this._splitWidget.installResizer(tabbedPane.headerElement());}
const stylesView=new View.SimpleView(UIString.UIString('Styles'));this.sidebarPaneView.appendView(stylesView);stylesView.element.classList.add('flex-auto');matchedStylePanesWrapper.show(stylesView.element);const computedView=new View.SimpleView(UIString.UIString('Computed'));computedView.element.classList.add('composite','fill');computedStylePanesWrapper.show(computedView.element);tabbedPane.addEventListener(TabbedPane.Events.TabSelected,tabSelected,this);this.sidebarPaneView.appendView(computedView);this._stylesViewToReveal=stylesView;this.sidebarPaneView.appendApplicableItems('elements-sidebar');const extensionSidebarPanes=self.Extensions.extensionServer.sidebarPanes();for(let i=0;i<extensionSidebarPanes.length;++i){this._addExtensionSidebarPane(extensionSidebarPanes[i]);}
...
const position=Settings.Settings.instance().moduleSetting('sidebarPosition').get();let splitMode=_splitMode.Horizontal;if(position==='right'||(position==='auto'&&self.UI.inspectorView.element.offsetWidth>680)){splitMode=_splitMode.Vertical;}
if(!this.sidebarPaneView){this._initializeSidebarPanes(splitMode);return;}
if(splitMode===this._splitMode){return;}
this._splitMode=splitMode;const tabbedPane=this.sidebarPaneView.tabbedPane();this._splitWidget.uninstallResizer(tabbedPane.headerElement());this._splitWidget.setVertical(this._splitMode===_splitMode.Vertical);this.showToolbarPane(null,null);if(this._splitMode!==_splitMode.Vertical){this._splitWidget.installResizer(tabbedPane.headerElement());}}
...
ElementsPanel._firstInspectElementCompletedForTest=function(){};const _splitMode={Vertical:Symbol('Vertical'),Horizontal:Symbol('Horizontal'),};class ContextMenuProvider{appendApplicableItems(event,contextMenu,object){if(!(object instanceof RemoteObject.RemoteObject&&((object)).isNode())&&!(object instanceof DOMModel.DOMNode)&&!(object instanceof DOMModel.DeferredDOMNode)){return;}
...
function checkRemoteObjectThenReveal(resolvedNode){if(!resolvedNode){const msg=ls`The remote object could not be resolved into a valid node.`;Console.Console.instance().warn(msg);reject(new Error(msg));return;}
onNodeResolved(resolvedNode);}
function checkDeferredDOMNodeThenReveal(resolvedNode){if(!resolvedNode){const msg=ls`The deferred DOM Node could not be resolved into a valid node.`;Console.Console.instance().warn(msg);reject(new Error(msg));return;}
onNodeResolved(resolvedNode);}}}}
...
_inspectNode(node){ElementsPanel.instance().revealAndSelectNode(node,true,true);}
...
async _getClassNames(selectedNode){const promises=[];const completions=new Set();this._selectedFrameId=selectedNode.frameId();const cssModel=selectedNode.domModel().cssModel();const allStyleSheets=cssModel.allStyleSheets();for(const stylesheet of allStyleSheets){if(stylesheet.frameId!==this._selectedFrameId){continue;}
const cssPromise=cssModel.classNamesPromise(stylesheet.id).then(classes=>{for(const className of classes){completions.add(className);}});promises.push(cssPromise);}
const domPromise=selectedNode.domModel().classNamesPromise(selectedNode.ownerDocument.id).then(classes=>{for(const className of classes){completions.add(className);}});promises.push(domPromise);await Promise.all(promises);return[...completions];}
...
var ClassesPaneWidget$1=Object.freeze({__proto__:null,ClassesPaneWidget:ClassesPaneWidget,ButtonProvider:ButtonProvider$1,ClassNamePrompt:ClassNamePrompt});class ElementStatePaneWidget extends Widget.Widget{constructor(){super(true);this.registerRequiredCSS('elements/elementStatePaneWidget.css');this.contentElement.className='styles-element-state-pane';this.contentElement.createChild('div').createTextChild(UIString.UIString('Force element state'));const table=document.createElement('table');table.classList.add('source-code');ARIAUtils.markAsPresentation(table);const inputs=[];this._inputs=inputs;function clickListener(event){const node=self.UI.context.flavor(DOMModel.DOMNode);if(!node){return;}
