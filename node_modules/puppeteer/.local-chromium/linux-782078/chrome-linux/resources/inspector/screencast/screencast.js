import{SDKModel,DOMModel,OverlayModel,ResourceTreeModel,NetworkManager,EmulationModel,ScreenCaptureModel}from'../sdk/sdk.js';import{UIString,Settings,Color,ResourceType}from'../common/common.js';import{Widget,ARIAUtils,KeyboardShortcut,UIUtils,Toolbar,RootView,SplitWidget}from'../ui/ui.js';import{InspectorFrontendHost}from'../host/host.js';class InputModel extends SDKModel.SDKModel{constructor(target){super(target);this._inputAgent=target.inputAgent();this._activeTouchOffsetTop=null;this._activeTouchParams=null;}
emitKeyEvent(event){let type;switch(event.type){case'keydown':type=Protocol.Input.DispatchKeyEventRequestType.KeyDown;break;case'keyup':type=Protocol.Input.DispatchKeyEventRequestType.KeyUp;break;case'keypress':type=Protocol.Input.DispatchKeyEventRequestType.Char;break;default:return;}
...
this._resourceTreeModel.navigate(encodeURI(decodeURI(url)));this._canvasElement.focus();}
...
InspectorFrontendHost.InspectorFrontendHostInstance.inspectedURLChanged(url);this._navigationUrl.value=decodeURI(url);}
...
const request=(event.data.request);if(request.type===ResourceType.resourceTypes.WebSocket){return;}
