...
_handleContextMenu(contextMenu){if(self.Components.dockController.canDock()){const dockItemElement=document.createElement('div');dockItemElement.classList.add('flex-centered');dockItemElement.classList.add('flex-auto');dockItemElement.tabIndex=-1;const titleElement=dockItemElement.createChild('span','flex-auto');titleElement.textContent=UIString.UIString('Dock side');const toggleDockSideShorcuts=self.UI.shortcutRegistry.shortcutsForAction('main.toggle-dock');titleElement.title=UIString.UIString('Placement of DevTools relative to the page. (%s to restore last position)',toggleDockSideShorcuts[0].title());dockItemElement.appendChild(titleElement);const dockItemToolbar=new Toolbar.Toolbar('',dockItemElement);if(Platform.isMac()&&!self.UI.themeSupport.hasTheme()){dockItemToolbar.makeBlueOnHover();}
...
if(descriptor['id']==='issues-pane'){moreTools.defaultSection().appendItem(extension.title(),()=>{userMetrics.issuesPanelOpenedFrom(UserMetrics.IssueOpener.HamburgerMenu);ViewManager.ViewManager.instance().showView('issues-pane',true);});continue;}
...
var SimpleApp$1=Object.freeze({__proto__:null,SimpleApp:SimpleApp,SimpleAppProvider:SimpleAppProvider});export{ExecutionContextSelector$1 as ExecutionContextSelector,MainImpl$1 as MainImpl,SimpleApp$1 as SimpleApp};