...
return block;}}
class GenericSettingsTab extends SettingsTab{constructor(){super(UIString.UIString('Preferences'),'preferences-tab-content');const explicitSectionOrder=['','Appearance','Sources','Elements','Network','Performance','Console','Extensions','Persistence','Debugger','Global'];const experimentalSections=[{name:'Grid',experiment:'cssGridFeatures'}];this._nameToSection=new Map();for(const sectionName of explicitSectionOrder){this._createSectionElement(sectionName);}
for(const section of experimentalSections){if(Root.Runtime.experiments.isEnabled(section.experiment)){this._createSectionElement(section.name);}}
...
const sectionElement=this._sectionElement(extension.descriptor()['category']);if(!sectionElement){return;}
const setting=Settings.Settings.instance().moduleSetting(extension.descriptor()['settingName']);const settingControl=SettingsUI.createControlForSetting(setting);if(settingControl){sectionElement.appendChild(settingControl);}}
_addSettingUI(extension){const descriptor=extension.descriptor();const sectionName=descriptor['category']||'';extension.instance().then(appendCustomSetting.bind(this));function appendCustomSetting(object){const settingUI=(object);const element=settingUI.settingElement();if(element){let sectionElement=this._sectionElement(sectionName);if(!sectionElement){sectionElement=this._createSectionElement(sectionName);}
sectionElement.appendChild(element);}}}
_createSectionElement(sectionName){const uiSectionName=sectionName&&UIString.UIString(sectionName);const sectionElement=this._appendSection(uiSectionName);this._nameToSection.set(sectionName,sectionElement);return sectionElement;}
_sectionElement(sectionName){return this._nameToSection.get(sectionName)||null;}}
...
var FrameworkBlackboxSettingsTab$1=Object.freeze({__proto__:null,FrameworkBlackboxSettingsTab:FrameworkBlackboxSettingsTab});class KeybindsSettingsTab extends Widget.VBox{constructor(){super(true);this.registerRequiredCSS('settings/keybindsSettingsTab.css');const header=this.contentElement.createChild('header');header.createChild('h1').textContent=ls`Shortcuts`;const keybindsSetSetting=self.Common.settings.moduleSetting('activeKeybindSet');keybindsSetSetting.addChangeListener(this.update,this);const keybindsSetSelect=SettingsUI.createControlForSetting(keybindsSetSetting,ls`Match shortcuts from preset`);keybindsSetSelect.classList.add('keybinds-set-select');this.contentElement.appendChild(keybindsSetSelect);this._items=new ListModel.ListModel();this._list=new ListControl.ListControl(this._items,this,ListControl.ListMode.NonViewport);this._items.replaceAll(this._createListItems());ARIAUtils.markAsList(this._list.element);this.registerRequiredCSS('settings/keybindsSettingsTab.css');this.contentElement.appendChild(this._list.element);this.update();}
createElementForItem(item){const itemElement=document.createElement('div');itemElement.classList.add('keybinds-list-item');ARIAUtils.markAsListitem(itemElement);itemElement.tabIndex=item===this._list.selectedItem()?0:-1;if(typeof item==='string'){itemElement.classList.add('keybinds-category-header');itemElement.textContent=item;}else{itemElement.createChild('div','keybinds-action-name keybinds-list-text').textContent=item.title();const shortcuts=self.UI.shortcutRegistry.shortcutsForAction(item.id());shortcuts.forEach((shortcut,index)=>{if(!shortcut.isDefault()){const icon=Icon.Icon.create('largeicon-shortcut-changed','keybinds-modified');ARIAUtils.setAccessibleName(icon,ls`Shortcut provided by preset`);itemElement.appendChild(icon);}
const shortcutElement=itemElement.createChild('div','keybinds-shortcut keybinds-list-text');const keys=shortcut.descriptors.flatMap(descriptor=>descriptor.name.split(' + '));keys.forEach(key=>{shortcutElement.createChild('span','keybinds-key').textContent=key;});});if(shortcuts.length===0&&self.UI.shortcutRegistry.actionHasDefaultShortcut(item.id())){const icon=Icon.Icon.create('largeicon-shortcut-changed','keybinds-modified');ARIAUtils.setAccessibleName(icon,ls`Shortcut provided by preset`);itemElement.appendChild(icon);}}
return itemElement;}
...
if(toElement){toElement.tabIndex=0;if(this._list.element.hasFocus()){toElement.focus();}}}
updateSelectedItemARIA(fromElement,toElement){return true;}
_createListItems(){const actions=self.UI.actionRegistry.actions().sort((actionA,actionB)=>{if(actionA.category()<actionB.category()){return-1;}
if(actionA.category()>actionB.category()){return 1;}
if(actionA.id()<actionB.id()){return-1;}
if(actionA.id()>actionB.id()){return 1;}
return 0;});const items=[];let currentCategory;actions.forEach(action=>{if(currentCategory!==action.category()){items.push(action.category());}
items.push(action);currentCategory=action.category();});return items;}
update(){this._list.refreshAllItems();if(!this._list.selectedItem()){this._list.selectItem(this._items.at(0));}}}
