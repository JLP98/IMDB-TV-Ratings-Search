...
const table=this._message.parameters&&this._message.parameters.length?this._message.parameters[0]:null;if(!table){return this._buildMessage();}
const actualTable=this._parameterToRemoteObject(table);if(!actualTable||!actualTable.preview){return this._buildMessage();}
const rawValueColumnSymbol=Symbol('rawValueColumn');const columnNames=[];const preview=actualTable.preview;const rows=[];for(let i=0;i<preview.properties.length;++i){const rowProperty=preview.properties[i];let rowSubProperties;if(rowProperty.valuePreview){rowSubProperties=rowProperty.valuePreview.properties;}else if(rowProperty.value){rowSubProperties=[{name:rawValueColumnSymbol,type:rowProperty.type,value:rowProperty.value}];}else{continue;}
...
if(columnRendered){const cellElement=this._renderPropertyPreviewOrAccessor(actualTable,[rowProperty,cellProperty]);cellElement.classList.add('console-message-nowrap-below');rowValue[cellProperty.name]=cellElement;}}
...
let currentStyle=null;function styleFormatter(obj){currentStyle={};const buffer=createElement('span');buffer.setAttribute('style',obj.description);for(let i=0;i<buffer.style.length;i++){const property=buffer.style[i];if(isAllowedProperty(property)){currentStyle[property]=buffer.style[property];}}}
function isAllowedProperty(property){const prefixes=['background','border','color','font','line','margin','padding','text','-webkit-background','-webkit-border','-webkit-font','-webkit-margin','-webkit-padding','-webkit-text'];for(let i=0;i<prefixes.length;i++){if(property.startsWith(prefixes[i])){return true;}}
return false;}
...
_onIssuesCountChanged(){if(IssuesManager.IssuesManager.instance().numberOfIssues()===0){if(this._issueBarDiv){this._issueBarDiv.remove();this._issueBarDiv=null;}}else if(!this._issueBarDiv&&!this._hasInteractedWithInfoBar){this._issueBarDiv=document.createElement('div');this._issueBarDiv.classList.add('flex-none');const issueBarAction=({text:ls`Go to Issues`,highlight:false,delegate:()=>{this._hasInteractedWithInfoBar=true;userMetrics.issuesPanelOpenedFrom(UserMetrics.IssueOpener.ConsoleInfoBar);ViewManager.ViewManager.instance().showView('issues-pane');},dismiss:true,});const issueBar=new Infobar.Infobar(Infobar.Type.Issue,ls`Issues detected. The new Issues tab displays information about deprecations, breaking changes and other potential problems.`,[issueBarAction]);issueBar.setCloseCallback(()=>{this._hasInteractedWithInfoBar=true;});this.element.insertBefore(this._issueBarDiv,this._consoleToolbarContainer.nextSibling);this._issueBarDiv.appendChild(issueBar.element);issueBar.setParentView(this);this.doResize();}}
...
const insertedInMiddle=insertAt<this._consoleMessages.length;this._consoleMessages.splice(insertAt,0,viewMessage);this._filter.onMessageAdded(message);this._sidebar.onMessageAdded(viewMessage);let shouldGoIntoGroup=false;const shouldGroupSimilar=this._groupSimilarSetting.get();if(message.isGroupable()){const groupKey=viewMessage.groupKey();shouldGoIntoGroup=shouldGroupSimilar&&this._groupableMessages.has(groupKey);let list=this._groupableMessages.get(groupKey);if(!list){list=[];this._groupableMessages.set(groupKey,list);}
list.push(viewMessage);}
this._computeShouldMessageBeVisible(viewMessage);if(!shouldGoIntoGroup&&!insertedInMiddle){this._appendMessageToEnd(viewMessage,!shouldGroupSimilar);this._updateFilterStatus();this._searchableView.updateSearchMatchesCount(this._regexMatchRanges.length);}else{this._needsFullUpdate=true;}
...
this._visibleViewMessages=[];if(this._groupSimilarSetting.get()){this._addGroupableMessagesToEnd();}else{for(let i=0;i<this._consoleMessages.length;++i){this._consoleMessages[i].setInSimilarGroup(false);this._appendMessageToEnd(this._consoleMessages[i],true);}}
...
if(!viewMessagesInGroup.find(x=>this._shouldMessageBeVisible(x))){SetUtilities.addAll(alreadyAdded,viewMessagesInGroup);processedGroupKeys.add(key);continue;}
