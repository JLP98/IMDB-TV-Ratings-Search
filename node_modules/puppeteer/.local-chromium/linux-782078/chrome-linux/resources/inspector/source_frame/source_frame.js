...
attachInfobar(infobar){if(!this._infoBarDiv){this._infoBarDiv=document.createElement('div');this._infoBarDiv.classList.add('flex-none');this.element.insertBefore(this._infoBarDiv,this.element.firstChild);}
...
hasLineClass(lineNumber,className){const lineInfo=this.codeMirror().lineInfo(lineNumber);if(!lineInfo){return false;}
const wrapClass=lineInfo.wrapClass;if(!wrapClass){return false;}
const classNames=wrapClass.split(' ');return classNames.indexOf(className)!==-1;}
...
_updateLineNumberFormatter(){if(this._wasmDisassembly){const disassembly=this._wasmDisassembly;const lastBytecodeOffset=disassembly.lineNumberToBytecodeOffset(disassembly.lineNumbers-1);const bytecodeOffsetDigits=lastBytecodeOffset.toString(16).length+1;this._textEditor.setLineNumberFormatter(lineNumber=>{const bytecodeOffset=disassembly.lineNumberToBytecodeOffset(lineNumber-1);return`0x${bytecodeOffset.toString(16).padStart(bytecodeOffsetDigits, '0')}`;});}else if(this._pretty){this._textEditor.setLineNumberFormatter(lineNumber=>{const line=this._prettyToRawLocation(lineNumber-1,0)[0]+1;if(lineNumber===1){return String(line);}
...
_updatePrettyPrintState(){this._prettyToggle.setToggled(this._pretty);this._textEditor.element.classList.toggle('pretty-printed',this._pretty);this._updateLineNumberFormatter();}
...
async _ensureContentLoaded(){if(!this._contentRequested){this._contentRequested=true;const progressIndicator=new ProgressIndicator.ProgressIndicator();progressIndicator.setTitle(UIString.UIString('Loading…'));progressIndicator.setTotalWork(100);this._progressToolbarItem.element.appendChild(progressIndicator.element);const{content,error}=(await this._lazyContent());this._rawContent=error||content||'';progressIndicator.setWorked(1);if(!error&&this._highlighterType==='application/wasm'){const worker=new Worker.WorkerWrapper('wasmparser_worker_entrypoint');const promise=new Promise((resolve,reject)=>{worker.onmessage=({data})=>{if('event'in data){switch(data.event){case'progress':progressIndicator.setWorked(data.params.percentage);break;}}else if('method'in data){switch(data.method){case'disassemble':resolve(data.result);break;}}};worker.onerror=reject;});worker.postMessage({method:'disassemble',params:{content}});const{source,offsets,functionBodyOffsets}=await promise;worker.terminate();this._rawContent=source;this._wasmDisassembly=new WasmDisassembly.WasmDisassembly(offsets,functionBodyOffsets);}
progressIndicator.setWorked(100);progressIndicator.done();this._formattedContentPromise=null;this._formattedMap=null;this._prettyToggle.setEnabled(true);if(error){this.setContent(null,error);this._prettyToggle.setEnabled(false);setTimeout(()=>this.setHighlighterType('text/plain'),50);}else{if(this._shouldAutoPrettyPrint&&TextUtils.isMinified(content||'')){await this._setPretty(true);}else{this.setContent(this._rawContent,null);}}}}
...
const{lineNumber,columnNumber}=this.uiLocationToEditorLocation(this._positionToReveal.line,this._positionToReveal.column);this._textEditor.revealPosition(lineNumber,columnNumber,this._positionToReveal.shouldHighlight);this._positionToReveal=null;}
...
if(mimeType==='application/wasm'){return'text/webassembly';}
return mimeType;}
...
if(this._wasmDisassembly){for(const lineNumber of this._wasmDisassembly.nonBreakableLineNumbers()){this._textEditor.toggleLineClass(lineNumber,'cm-non-breakable-line',true);}}
this._updateLineNumberFormatter();this._updateHighlighterType(content||'');this._wasShownOrLoaded();if(this._delayedFindSearchMatches){this._delayedFindSearchMatches();this._delayedFindSearchMatches=null;}
...
class Transformer{editorLocationToUILocation(lineNumber,columnNumber){throw new Error('Not implemented');}
uiLocationToEditorLocation(lineNumber,columnNumber){throw new Error('Not implemented');}}
var SourceFrame=Object.freeze({__proto__:null,SourceFrameImpl:SourceFrameImpl,LineDecorator:LineDecorator,Transformer:Transformer});class ResourceSourceFrame extends SourceFrameImpl{constructor(resource,autoPrettyPrint,codeMirrorOptions){super(async()=>{let content=(await resource.requestContent()).content||'';if(await resource.contentEncoded()){content=window.atob(content);}
...
_onFontContentLoaded(uniqueFontName,deferredContent){const{content}=deferredContent;const url=content?ContentProvider.contentAsDataURL(content,this._mimeType,true):this._url;this.fontStyleElement.textContent=StringUtilities.sprintf('@font-face { font-family: "%s"; src: url(%s); }',uniqueFontName,url);this.updateFontPreviewSize();}
...
const loadPromise=new Promise(x=>{this._imagePreviewElement.onload=x;});this._imagePreviewElement.src=imageSrc;const size=content&&!contentEncoded?content.length:base64ToSize(content);this._sizeLabel.setText(NumberUtilities.bytesToString(size));await loadPromise;this._dimensionsLabel.setText(UIString.UIString('%d × %d',this._imagePreviewElement.naturalWidth,this._imagePreviewElement.naturalHeight));}
