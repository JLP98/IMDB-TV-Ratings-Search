...
return result;};const FORMATTER_TYPES={STRING:'string',SPECIFIER:'specifier',};let FORMATTER_TOKEN;const tokenizeFormatString=function(formatString,formatters){const tokens=[];function addStringToken(str){if(!str){return;}
if(tokens.length&&tokens[tokens.length-1].type===FORMATTER_TYPES.STRING){tokens[tokens.length-1].value+=str;}else{tokens.push({type:FORMATTER_TYPES.STRING,value:str,specifier:undefined,precision:undefined,substitutionIndex:undefined});}}
function addSpecifierToken(specifier,precision,substitutionIndex){tokens.push({type:FORMATTER_TYPES.SPECIFIER,specifier,precision,substitutionIndex,value:undefined});}
...
tokens.push({type:FORMATTER_TYPES.SPECIFIER,specifier:'c',value:{description:(type.startsWith('bg')?'background : ':'color: ')+color},precision:undefined,substitutionIndex:undefined,});}
...
let result=initialValue;const tokens=tokenizedFormat||tokenizeFormatString(formatString,formatters);const usedSubstitutionIndexes={};const actualSubstitutions=substitutions||[];for(let i=0;i<tokens.length;++i){const token=tokens[i];if(token.type===FORMATTER_TYPES.STRING){result=append(result,token.value);continue;}
if(token.type!==FORMATTER_TYPES.SPECIFIER){error('Unknown token type "'+token.type+'" found.');continue;}
if(!token.value&&token.substitutionIndex!==undefined&&token.substitutionIndex>=actualSubstitutions.length){error('not enough substitution arguments. Had '+actualSubstitutions.length+' but needed '+
(token.substitutionIndex+1)+', so substitution was skipped.');result=append(result,'%'+((token.precision!==undefined&&token.precision>-1)?token.precision:'')+token.specifier);continue;}
if(!token.value&&token.substitutionIndex!==undefined){usedSubstitutionIndexes[token.substitutionIndex]=true;}
if(token.specifier===undefined||!(token.specifier in formatters)){warn('unsupported format character \u201C'+token.specifier+'\u201D. Treating as a string.');result=append(result,(token.value||token.substitutionIndex===undefined)?'':actualSubstitutions[token.substitutionIndex]);continue;}
result=append(result,formatters[token.specifier](token.value||(token.substitutionIndex!==undefined&&actualSubstitutions[token.substitutionIndex]),token));}
const unusedSubstitutions=[];for(let i=0;i<actualSubstitutions.length;++i){if(i in usedSubstitutionIndexes){continue;}
unusedSubstitutions.push(actualSubstitutions[i]);}
return{formattedResult:result,unusedSubstitutions:unusedSubstitutions};};const standardFormatters={d:function(substitution){return(!isNaN(substitution)?substitution:0);},f:function(substitution,token){if(substitution&&token.precision!==undefined&&token.precision>-1){substitution=substitution.toFixed(token.precision);}
const precision=(token.precision!==undefined&&token.precision>-1)?Number(0).toFixed(token.precision):0;return(!isNaN(substitution)?substitution:precision);},s:function(substitution){return(substitution);}};const vsprintf=function(formatString,substitutions){return format(formatString,substitutions,standardFormatters,'',function(a,b){return a+b;}).formattedResult;};const sprintf=function(format,var_arg){return vsprintf(format,Array.prototype.slice.call(arguments,1));};const toBase64=inputString=>{function encodeBits(b){return b<26?b+65:b<52?b+71:b<62?b-4:b===62?43:b===63?47:65;}
...
return result;};const collapseWhitespace=inputString=>{return inputString.replace(/[\s\xA0]+/g,' ');};const reverse=inputString=>{return inputString.split('').reverse().join('');};const replaceControlCharacters=inputString=>{return inputString.replace(/[\0-\x08\x0B\f\x0E-\x1F\x80-\x9F]/g,'\uFFFD');};const countWtf8Bytes=inputString=>{let count=0;for(let i=0;i<inputString.length;i++){const c=inputString.charCodeAt(i);if(c<=0x7F){count++;}else if(c<=0x07FF){count+=2;}else if(c<0xD800||0xDFFF<c){count+=3;}else{if(c<=0xDBFF&&i+1<inputString.length){const next=inputString.charCodeAt(i+1);if(0xDC00<=next&&next<=0xDFFF){count+=4;i++;continue;}}
count+=3;}}
return count;};const stripLineBreaks=inputStr=>{return inputStr.replace(/(\r)?\n/g,'');};const toTitleCase=inputStr=>{return inputStr.substring(0,1).toUpperCase()+inputStr.substring(1);};const removeURLFragment=inputStr=>{const url=new URL(inputStr);url.hash='';return url.toString();};var stringUtilities=Object.freeze({__proto__:null,escapeCharacters:escapeCharacters,FORMATTER_TOKEN:FORMATTER_TOKEN,tokenizeFormatString:tokenizeFormatString,format:format,standardFormatters:standardFormatters,vsprintf:vsprintf,sprintf:sprintf,toBase64:toBase64,findIndexesOfSubString:findIndexesOfSubString,findLineEndingIndexes:findLineEndingIndexes,isWhitespace:isWhitespace,trimURL:trimURL,collapseWhitespace:collapseWhitespace,reverse:reverse,replaceControlCharacters:replaceControlCharacters,countWtf8Bytes:countWtf8Bytes,stripLineBreaks:stripLineBreaks,toTitleCase:toTitleCase,removeURLFragment:removeURLFragment});String.sprintf=sprintf;String.regexSpecialCharacters=function(){return'^[]{}()\\.^$*+?|-,';};String.prototype.escapeForRegExp=function(){return escapeCharacters(this,String.regexSpecialCharacters());};String.filterRegex=function(query){const toEscape=String.regexSpecialCharacters();let regexString='';for(let i=0;i<query.length;++i){let c=query.charAt(i);if(toEscape.indexOf(c)!==-1){c='\\'+c;}
...
return this.substr(0,maxLength-1)+'…';};String.prototype.compareTo=function(other){if(this>other){return 1;}
if(this<other){return-1;}
return 0;};String.hashCode=function(string){if(!string){return 0;}
...
const number=Number(value);return number%1?number.toFixed(3):String(number);};(function(){const partition={value:function(comparator,left,right,pivotIndex){function swap(array,i1,i2){const temp=array[i1];array[i1]=array[i2];array[i2]=temp;}
...
return this.values().next().value;};Map.prototype.inverse=function(){const result=new Platform.Multimap();for(const key of this.keys()){const value=this.get(key);result.set(value,key);}
...
array.length=index;return true;};var arrayUtilities=Object.freeze({__proto__:null,removeElement:removeElement});const isValid=date=>{return!isNaN(date.getTime());};const toISO8601Compact=date=>{function leadZero(x){return(x>9?'':'0')+x;}
return date.getFullYear()+leadZero(date.getMonth()+1)+leadZero(date.getDate())+'T'+
leadZero(date.getHours())+leadZero(date.getMinutes())+leadZero(date.getSeconds());};var dateUtilities=Object.freeze({__proto__:null,isValid:isValid,toISO8601Compact:toISO8601Compact});function UIString(string,...vararg){return vsprintf(localize(string),Array.prototype.slice.call(arguments,1));}
...
class UIStringFormat{constructor(format){this._localizedFormat=localize(format);this._tokenizedFormat=tokenizeFormatString(this._localizedFormat,standardFormatters);}
static _append(a,b){return a+b;}
format(vararg){return format(this._localizedFormat,arguments,standardFormatters,'',UIStringFormat._append,this._tokenizedFormat).formattedResult;}}
...
var UIString$1=Object.freeze({__proto__:null,UIString:UIString,serializeUIString:serializeUIString,deserializeUIString:deserializeUIString,localize:localize,UIStringFormat:UIStringFormat,ls:ls});const clamp=(num,min,max)=>{let clampedNumber=num;if(num<min){clampedNumber=min;}else if(num>max){clampedNumber=max;}
return clampedNumber;};const mod=(m,n)=>{return((m%n)+n)%n;};const bytesToString=bytes=>{if(bytes<1000){return UIString('%.0f\xA0B',bytes);}
const kilobytes=bytes/1000;if(kilobytes<100){return UIString('%.1f\xA0kB',kilobytes);}
if(kilobytes<1000){return UIString('%.0f\xA0kB',kilobytes);}
const megabytes=kilobytes/1000;if(megabytes<100){return UIString('%.1f\xA0MB',megabytes);}
return UIString('%.0f\xA0MB',megabytes);};var numberUtilities=Object.freeze({__proto__:null,clamp:clamp,mod:mod,bytesToString:bytesToString});const addAll=function(set,iterable){for(const item of iterable){set.add(item);}};var setUtilities=Object.freeze({__proto__:null,addAll:addAll});const{ls:ls$1}=UIString$1;export{arrayUtilities as ArrayUtilities,dateUtilities as DateUtilities,Multimap,numberUtilities as NumberUtilities,setUtilities as SetUtilities,stringUtilities as StringUtilities,UIString$1 as UIString,ls$1 as ls};