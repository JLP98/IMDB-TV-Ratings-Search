...
const allowlist=new Set(['text/html','text/plain','application/xhtml+xml']);if(!allowlist.has(this.request.mimeType)){return null;}
...
const left=(scale*(range.start-startTime));right=(scale*(endTime-range.end));const duration=range.end-range.start;const tr=tableElement.createChild('tr');const timingBarTitleEement=tr.createChild('td');timingBarTitleEement.createTextChild(RequestTimingView._timeRangeTitle(rangeName));const row=tr.createChild('td').createChild('div','network-timing-row');const bar=row.createChild('span','network-timing-bar '+rangeName);bar.style.left=left+'%';bar.style.right=right+'%';bar.textContent='\u200B';ARIAUtils.setAccessibleName(row,ls`Started at ${calculator.formatValue(range.start, 2)}`);const label=tr.createChild('td').createChild('div','network-timing-bar-title');label.textContent=Number.secondsToString(duration,true);if(range.name==='serviceworker-respondwith'){timingBarTitleEement.classList.add('network-fetch-timing-bar-clickable');tableElement.createChild('tr','network-fetch-timing-bar-details');timingBarTitleEement.setAttribute('tabindex',0);timingBarTitleEement.setAttribute('role','switch');ARIAUtils.setChecked(timingBarTitleEement,false);}}
...
_constructFetchDetailsView(){if(!this._tableElement){return;}
const document=this._tableElement.ownerDocument;const fetchDetailsElement=document.querySelector('.network-fetch-timing-bar-details');if(!fetchDetailsElement){return;}
fetchDetailsElement.classList.add('network-fetch-timing-bar-details-collapsed');self.onInvokeElement(this._tableElement,this._onToggleFetchDetails.bind(this,fetchDetailsElement));const detailsView=new TreeOutline.TreeOutlineInShadow();fetchDetailsElement.appendChild(detailsView.element);const origRequest=NetworkLog.NetworkLog.instance().originalRequestForURL(this._request.url());if(origRequest){const requestObject=RemoteObject.RemoteObject.fromLocalObject(origRequest);const requestTreeElement=new ObjectUI.ObjectPropertiesSection.RootElement(requestObject);requestTreeElement.title=ls`Original Request`;detailsView.appendChild(requestTreeElement);}
const response=NetworkLog.NetworkLog.instance().originalResponseForURL(this._request.url());if(response){const responseObject=RemoteObject.RemoteObject.fromLocalObject(response);const responseTreeElement=new ObjectUI.ObjectPropertiesSection.RootElement(responseObject);responseTreeElement.title=ls`Response Received`;detailsView.appendChild(responseTreeElement);}
const serviceWorkerResponseSource=document.createElementWithClass('div','network-fetch-details-treeitem');let swResponseSourceString=ls`Unknown`;const swResponseSource=this._request.serviceWorkerResponseSource();if(swResponseSource){swResponseSourceString=this._getLocalizedResponseSourceForCode(swResponseSource);}
serviceWorkerResponseSource.textContent=ls`Source of response: ${swResponseSourceString}`;const responseSourceTreeElement=new TreeOutline.TreeElement(serviceWorkerResponseSource);detailsView.appendChild(responseSourceTreeElement);const cacheNameElement=document.createElementWithClass('div','network-fetch-details-treeitem');const responseCacheStorageName=this._request.getResponseCacheStorageCacheName();if(responseCacheStorageName){cacheNameElement.textContent=ls`Cache storage cache name: ${responseCacheStorageName}`;}else{cacheNameElement.textContent=ls`Cache storage cache name: Unknown`;}
const cacheNameTreeElement=new TreeOutline.TreeElement(cacheNameElement);detailsView.appendChild(cacheNameTreeElement);const retrievalTime=this._request.getResponseRetrievalTime();if(retrievalTime){const responseTimeElement=document.createElementWithClass('div','network-fetch-details-treeitem');responseTimeElement.textContent=ls`Retrieval Time: ${retrievalTime}`;const responseTimeTreeElement=new TreeOutline.TreeElement(responseTimeElement);detailsView.appendChild(responseTimeTreeElement);}}
_getLocalizedResponseSourceForCode(swResponseSource){switch(swResponseSource){case Protocol.Network.ServiceWorkerResponseSource.CacheStorage:return ls`ServiceWorker cache storage`;case Protocol.Network.ServiceWorkerResponseSource.HttpCache:return ls`From HTTP cache`;case Protocol.Network.ServiceWorkerResponseSource.Network:return ls`Network fetch`;default:return ls`Fallback code`;}}
_onToggleFetchDetails(fetchDetailsElement,event){if(!event.target){return;}
if(event.target.classList.contains('network-fetch-timing-bar-clickable')){const expanded=event.target.getAttribute('aria-checked')==='true';event.target.setAttribute('aria-checked',!expanded);fetchDetailsElement.classList.toggle('network-fetch-timing-bar-details-collapsed');fetchDetailsElement.classList.toggle('network-fetch-timing-bar-details-expanded');}}
...
this._tableElement=RequestTimingView.createTimingTable(this._request,this._calculator);this._tableElement.classList.add('resource-timing-table');this.element.appendChild(this._tableElement);if(this._request.fetchedViaServiceWorker){this._constructFetchDetailsView();}}}
...
return{box:anchor.boxInWindow(),show:async popover=>{this._popupLinkifier.setLiveLocationUpdateCallback(()=>{popover.setSizeBehavior(GlassPane.SizeBehavior.MeasureContent);});const content=RequestInitiatorView.createStackTracePreview((request),this._popupLinkifier,false);if(!content){return false;}
popover.contentElement.appendChild(content.element);return true;},hide:this._popupLinkifier.reset.bind(this._popupLinkifier)};}
