...
Linkifier._bindUILocation(anchor,uiLocation);const text=uiLocation.linkText(true);Linkifier._setTrimmedText(anchor,text,this._maxLength);let titleText=uiLocation.uiSourceCode.url();if(uiLocation.uiSourceCode.mimeType()==='application/wasm'){titleText+=`:0x${uiLocation.columnNumber.toString(16)}`;}else if(typeof uiLocation.lineNumber==='number'){titleText+=':'+(uiLocation.lineNumber+1);}
...
setLiveLocationUpdateCallback(callback){this._onLiveLocationUpdate=callback;}
