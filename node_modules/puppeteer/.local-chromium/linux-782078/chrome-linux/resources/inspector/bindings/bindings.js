import{ObjectWrapper,ResourceType,EventTarget,ParsedURL,Throttler,Revealer,Settings,Console,UIString}from'../common/common.js';import{ResourceTreeModel,SourceMapManager,RemoteObject,DebuggerModel,RuntimeModel,SourceMap,CompilerSourceMappingContentProvider,CSSModel,SDKModel,CSSStyleDeclaration,Script,ConsoleModel}from'../sdk/sdk.js';import{Workspace,UISourceCode,FileManager}from'../workspace/workspace.js';import{StaticContentProvider,TextRange,Text}from'../text_utils/text_utils.js';import{InspectorBackend}from'../protocol_client/protocol_client.js';import{StringUtilities}from'../platform/platform.js';class ContentProviderBasedProject extends Workspace.ProjectStore{constructor(workspace,id,type,displayName,isServiceProject){super(workspace,id,type,displayName);this._contentProviders={};this._isServiceProject=isServiceProject;workspace.addProject(this);}
async requestFileContent(uiSourceCode){const contentProvider=this._contentProviders[uiSourceCode.url()];try{const[content,isEncoded]=await Promise.all([contentProvider.requestContent(),contentProvider.contentEncoded()]);return{content:content.content,isEncoded,error:content.error};}catch(err){return{content:null,isEncoded:false,error:err?String(err):ls`Unknown error loading file`};}}
...
const attributionInfo=frameAttribution.get(frameId);console.assert(attributionInfo,'Failed to remove frame attribution for url: '+uiSourceCode.url());if(!attributionInfo){return;}
attributionInfo.count-=1;if(attributionInfo.count>0){return;}
...
var CompilerScriptMapping$1=Object.freeze({__proto__:null,CompilerScriptMapping:CompilerScriptMapping});class SourceVariable extends RemoteObject.RemoteObjectImpl{constructor(callFrame,variable,plugin,location){super(callFrame.debuggerModel.runtimeModel(),undefined,variable.type,undefined,null,undefined,variable.type);this._variable=variable;this._callFrame=callFrame;this._plugin=plugin;this._location=location;this._hasChildren=true;this._evaluator=plugin.evaluateVariable(variable.name,location);}
get type(){return this._variable.type;}
...
const repr=await getRepr(this._evaluator,this._callFrame,this._plugin);return({properties:[new RemoteObject.RemoteObjectProperty('value',repr,false,false,true,false)],internalProperties:[]});async function getRepr(evaluatorPromise,callFrame,plugin){try{const evaluator=await evaluatorPromise;const evaluateResponse=await callFrame.debuggerModel._agent.invoke_executeWasmEvaluator({callFrameId:callFrame.id,evaluator:evaluator.code});if(evaluateResponse[InspectorBackend.ProtocolError]||evaluateResponse.exceptionDetails){console.error(evaluateResponse[InspectorBackend.ProtocolError]||evaluateResponse.exceptionDetails.exception.description);return null;}
const value=JSON.parse(evaluateResponse.result.value);return await plugin.getRepresentation(value);}catch(error){if(!(error instanceof DebuggerLanguagePluginError)){throw error;}
console.error('Error in debugger language plugin: '+error.message+' ('+error.code+')');return null;}}}}
class SourceScopeRemoteObject extends RemoteObject.RemoteObjectImpl{constructor(callFrame,plugin,location){super(callFrame.debuggerModel.runtimeModel(),undefined,'object',undefined,null);this.variables=[];this._callFrame=callFrame;this._plugin=plugin;this._location=location;}
async doGetProperties(ownProperties,accessorPropertiesOnly,generatePreview){if(accessorPropertiesOnly){return({properties:[],internalProperties:[]});}
const properties=this.variables.map(variable=>new RemoteObject.RemoteObjectProperty(variable.name,new SourceVariable(this._callFrame,variable,this._plugin,this._location),false,false,true,false));return({properties:properties,internalProperties:[]});}}
class SourceScope{constructor(callFrame,type,plugin,location){this._callFrame=callFrame;this._type=type;this._object=new SourceScopeRemoteObject(callFrame,plugin,location);this._name=type;this._startLocation=null;this._endLocation=null;}
...
async resolveScopeChain(callFrame){if(!callFrame){return null;}
const script=callFrame.script;const plugin=this._pluginForScriptId.get(script.scriptId);if(!plugin){return null;}
const scopes=new Map();const location={'rawModuleId':script.scriptId,'codeOffset':callFrame.location().columnNumber-script.codeOffset()};const variables=await plugin.listVariablesInScope(location);if(variables){for(const variable of variables){if(!scopes.has(variable.scope)){scopes.set(variable.scope,new SourceScope(callFrame,variable.scope,plugin,location));}
...
let RawModule;let RawLocation;let SourceLocation;let Variable;let VariableValue;class DebuggerLanguagePlugin{handleScript(script){}
dispose(){}
...
async evaluateVariable(name,location){return(await _sendJsonRPC('evaluateVariable',{name:name,location:location})).value;}
dispose(){}
_reprString(value){return value.value;}
_reprNumber(value){return Number(value.value);}
_reprCompound(value){const result={};for(const property of value.value){result[property.name]=this._repr(property);}
return result;}
_reprArray(value){if(value.value.length>0&&value.value[0].name&&value.value[0].name.endsWith(']')){return value.value.map(v=>this._repr(v));}
return this._reprCompound(value);}
_repr(value){if(Array.isArray(value.value)){return this._reprArray(value);}
console.error(`Repr for type ${value.type}`);const numberTypes=['int8_t','int16_t','int32_t','int64_t','uint8_t','uint16_t','uint32_t','uint64_t','float','double','long double'];if(numberTypes.indexOf(value.type)>-1){return this._reprNumber(value);}
return this._reprString(value);}
async getRepresentation(value){return new RemoteObject.LocalJSONObject(this._repr(value));}}
...
async _styleSheetChanged(event){const header=this._cssModel.styleSheetHeaderForId(event.data.styleSheetId);if(!header||!header.isInline||(header.isInline&&header.isMutable)){return;}
...
const binding=this._bindings.get(resource.url);if(!binding){continue;}
if(binding._resources.size===1){binding.dispose();this._bindings.delete(resource.url);}else{binding.removeResource(resource);}}}
...
hasBoundScript(){for(const uiSourceCode of this._uiSourceCodes){if(uiSourceCode.project().type()===Workspace.projectTypes.Network){return true;}}
return false;}
...
this._debuggerId=breakpointId;this._debuggerModel.addBreakpointListener(this._debuggerId,event=>this._breakpointResolved(event),this);for(const location of locations){if(!(await this._addResolvedLocation(location))){break;}}
callback();}
_didRemoveFromDebugger(){if(this._cancelCallback){this._cancelCallback=false;return;}
this._resetLocations();this._debuggerModel.removeBreakpointListener(this._debuggerId,event=>this._breakpointResolved(event),this);delete this._debuggerId;}
...
const uiSourceCode=project.createUISourceCode(url,originalContentProvider.contentType());NetworkProject.setInitialFrameAttribution(uiSourceCode,script.frameId);const metadata=metadataForURL(this._debuggerModel.target(),script.frameId,url);const scriptFile=new ResourceScriptFile(this,uiSourceCode,[script]);this._uiSourceCodeToScriptFile.set(uiSourceCode,scriptFile);const mimeType=script.isWasm()?'application/wasm':'text/javascript';project.addUISourceCodeWithProvider(uiSourceCode,originalContentProvider,metadata,mimeType);await this._debuggerWorkspaceBinding.updateLocations(script);}
...
const debuggerModel=this._resourceScriptMapping._debuggerModel;const breakpoints=BreakpointManager.instance().breakpointLocationsForUISourceCode(this._uiSourceCode).map(breakpointLocation=>breakpointLocation.breakpoint);const source=this._uiSourceCode.workingCopy();debuggerModel.setScriptSource(this._script.scriptId,source,(error,exceptionDetails)=>{this.scriptSourceWasSet(source,breakpoints,error,exceptionDetails);});}
async scriptSourceWasSet(source,breakpoints,error,exceptionDetails){if(!error&&!exceptionDetails){this._scriptSource=source;}
...
const mappings=sourceMap.mappings();const newRanges=[];if(mappings.length>0){let currentBlackboxed=false;if(mappings[0].lineNumber!==0||mappings[0].columnNumber!==0){newRanges.push({lineNumber:0,columnNumber:0});currentBlackboxed=true;}
