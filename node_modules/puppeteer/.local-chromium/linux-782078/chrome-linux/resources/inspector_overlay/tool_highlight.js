...
const GridArrowTypes={leftTop:'left-top',leftMid:'left-mid',leftBottom:'left-bottom',topLeft:'top-left',topMid:'top-mid',topRight:'top-right',rightTop:'right-top',rightMid:'right-mid',rightBottom:'right-bottom',bottomLeft:'bottom-left',bottomMid:'bottom-mid',bottomRight:'bottom-right',};const gridArrowWidth=3;const gridPageMargin=20;const gridLabelDistance=20;function*offsetIterator(offsets){let lastEmittedOffset=null;for(const[i,offset]of offsets.entries()){const isFirst=i===0;const isLast=i===offsets.length-1;const isFarEnoughFromPrevious=offset-lastEmittedOffset>gridLabelDistance;const isFarEnoughFromLast=!isLast&&offsets[offsets.length-1]-offset>gridLabelDistance;if(isFirst||isLast||(isFarEnoughFromPrevious&&isFarEnoughFromLast)){yield[i,offset];lastEmittedOffset=offset;}}}
function drawGridNumbers(config,bounds){const labelContainer=document.getElementById('grid-label-container');labelContainer.removeChildren();if(config.gridHighlightConfig.showPositiveLineNumbers&&config.positiveColumnLineNumberOffsets){for(const[i,offset]of offsetIterator(config.positiveColumnLineNumberOffsets)){const element=_createLabelElement(labelContainer,i+1);_placePositiveColumnLabel(element,offset,config,bounds);}}
if(config.gridHighlightConfig.showPositiveLineNumbers&&config.positiveRowLineNumberOffsets){for(const[i,offset]of offsetIterator(config.positiveRowLineNumberOffsets)){const element=_createLabelElement(labelContainer,i+1);_placePositiveRowLabel(element,offset,config,bounds);}}
if(config.gridHighlightConfig.showNegativeLineNumbers&&config.negativeColumnLineNumberOffsets){for(const[i,offset]of offsetIterator(config.negativeColumnLineNumberOffsets)){const element=_createLabelElement(labelContainer,config.negativeColumnLineNumberOffsets.length*-1+i);_placeNegativeColumnLabel(element,offset,config,bounds);}}
if(config.gridHighlightConfig.showNegativeLineNumbers&&config.negativeRowLineNumberOffsets){for(const[i,offset]of offsetIterator(config.negativeRowLineNumberOffsets)){const element=_createLabelElement(labelContainer,config.negativeRowLineNumberOffsets.length*-1+i);_placeNegativeRowLabel(element,offset,config,bounds);}}}
...
function _placePositiveRowLabel(element,offset,config,bounds){const x=bounds.minX;const y=bounds.minY+offset;const isAtSharedStartCorner=offset===0&&config.gridHighlightConfig.showPositiveLineNumbers&&config.positiveColumnLineNumberOffsets&&config.positiveColumnLineNumberOffsets[0]===0;const isAtSharedEndCorner=offset===bounds.maxY-bounds.minY&&config.gridHighlightConfig.showNegativeLineNumbers&&config.negativeColumnLineNumberOffsets&&config.negativeColumnLineNumberOffsets[0]===0;const isTooCloseToViewportStart=y<gridPageMargin;const isTooCloseToViewportEnd=canvasHeight-y<gridPageMargin;const flipIn=x<gridPageMargin;if(flipIn&&(isAtSharedStartCorner||isAtSharedEndCorner)){element.classList.add('inner-shared-corner');}
...
arrowType=_flipArrowTypeIfNeeded(arrowType,flipIn);_placeLabel(element,arrowType,x,y);}
function _placeNegativeRowLabel(element,offset,config,bounds){const x=bounds.maxX;const y=bounds.minY+offset;const isAtSharedStartCorner=offset===0&&config.gridHighlightConfig.showPositiveLineNumbers&&config.positiveColumnLineNumberOffsets&&config.positiveColumnLineNumberOffsets[config.positiveColumnLineNumberOffsets.length-1]===bounds.maxX-bounds.minX;const isAtSharedEndCorner=offset===bounds.maxY-bounds.minY&&config.gridHighlightConfig.showNegativeLineNumbers&&config.negativeColumnLineNumberOffsets&&config.negativeColumnLineNumberOffsets[config.negativeColumnLineNumberOffsets.length-1]===bounds.maxX-bounds.minX;const isTooCloseToViewportStart=y<gridPageMargin;const isTooCloseToViewportEnd=canvasHeight-y<gridPageMargin;const flipIn=canvasWidth-x<gridPageMargin;if(flipIn&&(isAtSharedStartCorner||isAtSharedEndCorner)){element.classList.add('inner-shared-corner');}
...
arrowType=_flipArrowTypeIfNeeded(arrowType,flipIn);_placeLabel(element,arrowType,x,y);}
function _placePositiveColumnLabel(element,offset,config,bounds){const x=bounds.minX+offset;const y=bounds.minY;const isAtSharedStartCorner=offset===0&&config.gridHighlightConfig.showPositiveLineNumbers&&config.positiveRowLineNumberOffsets&&config.positiveRowLineNumberOffsets[0]===0;const isAtSharedEndCorner=offset===bounds.maxX-bounds.minX&&config.gridHighlightConfig.showNegativeLineNumbers&&config.negativeRowLineNumberOffsets&&config.negativeRowLineNumberOffsets[0]===0;const isTooCloseToViewportStart=x<gridPageMargin;const isTooCloseToViewportEnd=canvasWidth-x<gridPageMargin;const flipIn=y<gridPageMargin;if(flipIn&&(isAtSharedStartCorner||isAtSharedEndCorner)){element.classList.add('inner-shared-corner');}
...
arrowType=_flipArrowTypeIfNeeded(arrowType,flipIn);_placeLabel(element,arrowType,x,y);}
function _placeNegativeColumnLabel(element,offset,config,bounds){const x=bounds.minX+offset;const y=bounds.maxY;const isAtSharedStartCorner=offset===0&&config.gridHighlightConfig.showPositiveLineNumbers&&config.positiveRowLineNumberOffsets&&config.positiveRowLineNumberOffsets[config.positiveRowLineNumberOffsets.length-1]===bounds.maxY-bounds.minY;const isAtSharedEndCorner=offset===bounds.maxX-bounds.minX&&config.gridHighlightConfig.showNegativeLineNumbers&&config.negativeRowLineNumberOffsets&&config.negativeRowLineNumberOffsets[config.negativeRowLineNumberOffsets.length-1]===bounds.maxY-bounds.minY;const isTooCloseToViewportStart=x<gridPageMargin;const isTooCloseToViewportEnd=canvasWidth-x<gridPageMargin;const flipIn=canvasHeight-y<gridPageMargin;if(flipIn&&(isAtSharedStartCorner||isAtSharedEndCorner)){element.classList.add('inner-shared-corner');}
